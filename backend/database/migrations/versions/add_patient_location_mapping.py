...
def upgrade() -> None:
    """Upgrade schema."""
    op.add_column(
        "patients", sa.Column("clinic_id", sa.String(), nullable=True)
    )

    op.add_column(
        "patients", sa.Column("position_id", sa.String(), nullable=True)
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_patients_position_id", "patients", type_="foreignkey"
    )
