...
import { buildOptimisticProperties, readEntityProperties, applyOptimisticPropertyScalars, getNewOptimisticProperties, type OptimisticPropertyValue } from './optimisticProperties'
...
describe('buildOptimisticProperties', () => {
  ...
  it('partitions new attachment properties from persisted ones', () => {
    const properties = [
      makeProp({ id: 'real-uuid-1', definitionId: 'def-1', textValue: 'a' }),
      makeProp({ id: 'attachment-patient-1-def-2', definitionId: 'def-2', textValue: 'b' }),
    ]
    expect(getNewOptimisticProperties(properties).map((property) => property.id)).toEqual([
      'attachment-patient-1-def-2',
    ])
  })
})
